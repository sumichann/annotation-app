...
export type { AssignmentProgress, ProgressResponse } from '../types/progress'
    ...
    const handleLoadProgress = async () => {
        ...
        try {
            const res = await fetch(
                `${getApiUrl()}/progress?username=${encodeURIComponent(name)}`
            )
    ...
    const handleResume = async (category: string, nextIndex: number) => {
        ...
        try {
            ...
            if (data?.anon_item_id) {
                ...
                if (progress?.username) params.set('username', progress.username)
                navigate(`/item/${data.anon_item_id}?${params.toString()}`)
            }
        } catch (err) {
