    ...
    const handleStartAnnotation = async () => {
        ...
        try {
            const res = await fetch(
                `${getApiUrl()}/items/by-index?category=${encodeURIComponent(selectedCategory)}&index=${index}`
            )
            if (!res.ok) {
                if (res.status === 404) {
...
export default StartByIndexPage
